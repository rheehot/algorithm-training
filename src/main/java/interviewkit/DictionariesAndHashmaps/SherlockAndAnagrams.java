 ...
 *
 * 3. ifailuhkqq => [i,i] [g,g] [ifa, fai] => 처럼 3가지다 ifa, fai 처럼 같은 문자열포함 쌍인데 ifa = fia 이렇게 순서를 변경한 케이스는 없다
 * 4. kkkk => [k,k] [k,k] [k,k] [k,k]
 *            [1,2] [2,3] [3,4] [1,4] 처럼 개별 자리의 문자로 인식한다
 *    즉 [k,k]가 있다고 [k,k] 케이스가 또있네 라고 햇갈릴수 있는데
 *    각각 개별 자리의 문자를 독립적으로 본다
 * 5. aaa을 때 각각 a, aa, aaa의 조합 케이스를 map에 key = 문자, value = 조합 회수를 저장 해 놓는
 *
 * 6. n(n-1)/2 공식을 적용하여 aaa 3일때 각 조합
 *    a 는 3개 이고 공식 적용하면 = 3
 *    aa 는 2개 이고 공식적용 = 1
 *    aaa 는 1개라 공식적용 = 0
 *    1+3= 모두 더하면 4가 된다.
    ...
    public static int sherlockAndAnagrams(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            for (int j = i+1; j < s.length()+1; j++) {
                String str = Stream.of(s.substring(i, j).split("")).sorted().collect(Collectors.joining());
