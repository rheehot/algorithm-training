...
public class SpecialStringAgain {
    ...
    public static long substrCount(int n, String s) {
        s = s + "1";
        long count = 1, sum = 0;
        List<long[]> strFreq = new ArrayList<>();

        for (int i = 0; i < s.length() - 1; i++) {
            if (s.charAt(i) == s.charAt(i + 1)) {
                count++;
            } else {
                strFreq.add(new long[]{s.charAt(i), count});
                sum += count * (count + 1) / 2;
                count = 1;
            }
        }
        ...
        for (int i = 1; i < strFreq.size() - 1; i++) {
            if (strFreq.get(i)[1] == 1 && strFreq.get(i - 1)[0] == strFreq.get(i + 1)[0]) {
                sum += Math.min(strFreq.get(i - 1)[1], strFreq.get(i + 1)[1]);
            }
        }
